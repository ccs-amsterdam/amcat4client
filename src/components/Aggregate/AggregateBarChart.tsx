...
export default function AggregateBarChart({ data, onClick, width, height, limit }: AggregateVisualizerProps) {
  ...
  if (can_transform(data.meta.axes[0].interval)) {
    sorted = d.map((x) => transform_dateparts(x, data.meta.axes[0])).sort((e1, e2) => e1._sort - e2._sort);
  } else {
    ...
    <ResponsiveContainer width={width} height={height}>
      <BarChart data={sorted} layout="vertical">
        ...
        {columns.map((column, i) => (
          <Bar key={i} type="monotone" dataKey={column} fill={colors[i]} onClick={(e, j) => handleClick(column, j)} />
        ))}
      </BarChart>
    </ResponsiveContainer>
  );
